...
% Make sure no .txt files are present when running this script!
...
for i = 1:length(mol)
    ...
    if length(cell{1}) == mx
        ...
        mol_new = horzcat(mol_new,cell{1});
    else
        mol_new = horzcat(mol_new,cell{1});
    end
end
...
mol_cell = num2cell(mol_new);
mol_new = [label;mol_cell];

% Save to tab-delimited .txt file
ind = strfind(file,'.');
file(ind+1:end)=[];
fout = strcat(file,'txt');
dlmcell(fout,mol_new);
